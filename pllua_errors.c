...
void luatable_report(lua_State *L, int elevel)
{
    ...
    ereport(elevel,
            (errcode(edata.sqlerrcode ? edata.sqlerrcode
                                      : ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
             errmsg_internal(
                 "%s", edata.message ? edata.message : "no exception data"),
...
/* this is a copy from lua source to call debug.traceback
 * without access it from metatables */
