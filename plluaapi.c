...
#define datum2string(d, f) DatumGetCString(DirectFunctionCall1((f), (d)))
...
static int luaP_datumsave(lua_State *L)
{
    ...
    if (d == NULL) {
        const char *msg = lua_pushfstring(L, "%s expected, got %s", PLLUA_DATUM,
                                          luaL_typename(L, 1));
...
static luaP_Datum *luaP_pushrawdatum(lua_State *L, Datum dat, luaP_Typeinfo *ti)
{
...
static void luaP_preptrigger(lua_State *L, TriggerData *tdata)
{
    ...
    if (TRIGGER_FIRED_FOR_ROW(tdata->tg_event)) {
        if (TRIGGER_FIRED_BY_UPDATE(tdata->tg_event)) {
            luaP_pushtuple_trg(L, tdata->tg_relation->rd_att,
                               tdata->tg_newtuple, tdata->tg_relation->rd_id,
                               0);
...
static int luaP_modinit(lua_State *L)
{
    ...
    if (SPI_processed == 0) { /* pllua.init does not exist? */
        ...
        if (status < 0) {
            lua_pushfstring(
                L, "[pllua]: error loading modules from pllua.init: %d",
                status);
        } else {
            unsigned int i;
            status = 0;
            ...
            for (i = 0; i < SPI_processed; i++) {
                ...
                /* push module name */
                lua_pushstring(
                    L, text2string(heap_getattr(SPI_tuptable->vals[i], 1,
...
{
    return luaL_error(L, "attempt to set global var '%s'", lua_tostring(L, -2));
}
...
static int luaP_fromstring(lua_State *L)
{
    ...
    if (oid == InvalidOid) {
        ...
        if (ti->type == TYPTYPE_BASE && OidIsValid(ti->elem)) {
            inoid = ti->elem;
        }
...
#ifdef PLLUA_DEBUG
static int luaP_memstat(lua_State *L)
{
    (void)L;
...
static const luaL_Reg luaP_funcs[] = {{"assert", luaB_assert},
                                      {"error", luaB_error},
...
lua_State *luaP_newstate(int trusted)
{
    ...
    MemoryContext mcxt = AllocSetContextCreate(
        TopMemoryContext, "PL/Lua context", ALLOCSET_DEFAULT_MINSIZE,
    ...
    if (trusted) {
        const luaL_Reg luaP_trusted_libs[] = {
            ...
            {LUA_LOADLIBNAME,
             luaopen_package}, /* just for pllua.init modules */
            {NULL, NULL}
        };
                               ...
                               Form_pg_proc procst)
{
    ...
    int i;
    luaP_Typeinfo *ti;
    bool code_storage = ((nargs == 1) && (argtype[0] == INTERNALOID) &&
                         (rettype == INTERNALOID));
    ...
    if (!code_storage) {
        ...
        if (ti->type == TYPTYPE_PSEUDO && rettype != VOIDOID &&
            rettype != TRIGGEROID) {
                             ...
                             luaP_Info **fi)
{
    ...
    text *t;
    luaL_Buffer b;
    int nargs, i, init = (*fi == NULL);
    const char *source, *fname, *chunk_name = NULL; /*  PLLUA_CHUNKNAME; */
    bool isnull;
    ...
    if ((nargs > 0) && ((*fi)->code_storage == 0)) {
        ...
        if (nnames != nargs) {
            ...
            for (i = 0; i < nnames && !(*fi)->vararg; i++) {
                if (VARSIZE(DatumGetTextP(argname[i])) == VARHDRSZ) {
                    (*fi)->vararg = 1;
                }
            }
        }
...
static void luaP_pusharray(lua_State *L, char **p, int ndims, int *dims,
                           int *lb, bits8 **bitmap, int *bitmask,
    ...
    default: {
        ...
        case TYPTYPE_DOMAIN:
            if (ti->elem != 0 && ti->len == -1) { /* array? */
                ...
                luaP_pusharray(L, &p, ARR_NDIM(arr), ARR_DIMS(arr),
                               ARR_LBOUND(arr), &bitmap, &bitmask, te,
                               ti->elem);
            } else {
...
#if PG_VERSION_NUM >= 80300
        case TYPTYPE_ENUM:
            lua_pushstring(L, (const char *)DatumGetCString(
                                  DirectFunctionCall1(enum_out, dat)));
            break;
#endif
        default:
            argerror(type);
...
static void luaP_pushargs(lua_State *L, FunctionCallInfo fcinfo, luaP_Info *fi)
{
    int i;
    for (i = 0; i < fcinfo->nargs; i++) {
        if (fcinfo->argnull[i]) {
            ...
            luaP_pushdatum(L, fcinfo->arg[i], fi->arg[i]);
        }
    }
}
...
static int luaP_getarraydims(lua_State *L, int *ndims, int *dims, int *lb,
                             ...
                             bool *hasnulls)
{
    ...
    *hasnulls = false;
    ...
    while (lua_next(L, -2)) {
        if (lua_isnumber(L, -2)) {
            ...
            if (lua_type(L, -1) == LUA_TTABLE) {
                ...
                if (*ndims == MAXDIM) {
                    elog(ERROR,
                         "[pllua]: table exceeds max number of dimensions");
                }
                ...
                if (size > MaxAllocSize) {
                    elog(ERROR,
                         "[pllua]: array size exceeds the maximum allowed");
                }
            }
            ...
            if (*ndims < 0) {
                *ndims = n;
                ...
                elog(ERROR, "[pllua]: table is asymetric");
            }
        }
        nitems++;
        lua_pop(L, 1);
    ...
    if (!(*hasnulls)) {
        *hasnulls = (nitems > 0 && nitems != *dims);
    }

    return size;
}

static void luaP_toarray(lua_State *L, char **p, int ndims, int *dims, int *lb,
                         bits8 **bitmap, int *bitmask, int *bitval,
                         luaP_Typeinfo *ti, Oid typeelem, int typmod)
{
    int i;
    bool isnull;
    if (ndims == 1) { /* vector? */
        ...
        for (i = 0; i < (*dims); i++) {
            lua_rawgeti(L, -1, (*lb) + i);
            luaP_toarray(L, p, ndims - 1, dims + 1, lb + 1, bitmap, bitmask,
                         bitval, ti, typeelem, typmod);
            lua_pop(L, 1);
        }
    }
}
        ...
        default: {
            ...
            case TYPTYPE_COMPOSITE:
                if (lua_type(L, idx) == LUA_TTABLE) {
                    ...
                    if (lua_type(L, idx) != LUA_TTABLE) {
                        elog(
                            ERROR,
                            "[pllua]: table expected for record result, got %s",
                            lua_typename(L, lua_type(L, idx)));
                    }
                    ...
                    for (i = 0; i < ti->tupdesc->natts; i++) {
                        lua_getfield(L, idx,
                                     NameStr(ti->tupdesc->attrs[i]->attname));
                        /* only simple types allowed in record */
                        b->value[i] = luaP_todatum(
                            L, ti->tupdesc->attrs[i]->atttypid,
                            ti->tupdesc->attrs[i]->atttypmod, b->null + i, idx);
                        lua_pop(L, 1);
                    }
                    /* make copy in upper executor memory context */
                    dat = PointerGetDatum(SPI_returntuple(
                        heap_form_tuple(ti->tupdesc, b->value, b->null),
                        ti->tupdesc));
                } else { /* tuple */
                    HeapTuple tuple = luaP_casttuple(L, ti->tupdesc);
                    if (tuple == NULL) {
                        elog(ERROR, "[pllua]: table or tuple expected for "
                                    "record result, got %s",
                             lua_typename(L, lua_type(L, idx)));
                    }
            ...
            case TYPTYPE_DOMAIN:
                if (ti->elem != 0 && ti->len == idx) { /* array? */
                    luaP_Typeinfo *te;
                    int ndims, dims[MAXDIM], lb[MAXDIM], i, size;
                    bool hasnulls;
                    ArrayType *a;
                    if (lua_type(L, idx) != LUA_TTABLE) {
                        elog(ERROR, "[pllua]: table expected for array "
                                    "conversion, got %s",
                             lua_typename(L, lua_type(L, idx)));
                    }
                    ...
                    if (size == 0) { /* empty array? */
                        ...
                        int offset, nitems = 1, bitmask = 1, bitval = 0;
                        char *p;
                        bits8 *bitmap;
                        ...
                        for (i = 0; i < ndims; i++) {
                            ...
                            if (nitems > MaxArraySize) {
                                elog(ERROR, "[pllua]: array size exceeds "
                                            "maximum allowed");
                            }
                        }
                    ...
                    if (d == NULL) {
                        elog(ERROR, "[pllua]: raw datum expected for datum "
                                    "conversion, got %s",
                             lua_typename(L, lua_type(L, idx)));
                    }
...
static Datum luaP_getresult(lua_State *L, FunctionCallInfo fcinfo, Oid type)
{
...
Datum luaP_callhandler(lua_State *L, FunctionCallInfo fcinfo)
{
    ...
    PG_TRY();
    {
        ...
        if ((fi->result == TRIGGEROID && !istrigger) ||
            ...
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                            errmsg("[pllua]: trigger function can only be "
                                   "called as trigger")));
        }
            ...
            if (fi->result_isset) { /* SETOF? */
                ...
                if (fi->L == NULL) { /* first call? */
                    if (!rsi || !IsA(rsi, ReturnSetInfo) ||
                        ...
                        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                                        errmsg("[pllua]: set-valued function "
                                               "called in context"
