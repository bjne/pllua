...
#define ENDLUA                                                                 \
    ____stk = lua_gettop(L) - ____stk;                                         \
    if (0 != ____stk)                                                          \
    ereport(INFO, (errmsg("stk %s>%i", AT, ____stk)))

#define ENDLUAV(v)                                                             \
    ____stk = (lua_gettop(L) - ____stk - v);                                   \
    if (0 != ____stk)                                                          \
    ereport(INFO, (errmsg("stk %s>%i", AT, ____stk)))
#else
#define BEGINLUA
...
#define lua_push_oidstring(L, oid)                                             \
    do {                                                                       \
        luaL_Buffer b;                                                         \
        luaL_buffinit(L, &b);                                                  \
