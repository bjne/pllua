 ...
 */
#define WRAP_SUBTRANSACTION(source_code)                                       \
    do {                                                                       \
        ...
        stb_enter(L, &subtran);                                                \
        PG_TRY();                                                              \
        {source_code} PG_CATCH();                                              \
        {                                                                      \
            ErrorData *edata;                                                  \
            edata = CopyErrorData();                                           \
            ereport(FATAL,                                                     \
                    (errmsg("Unhandled exception: %s", edata->message)));      \
        }                                                                      \
        PG_END_TRY();                                                          \
...
int subt_luaB_xpcall(lua_State *L)
{
    ...
    WRAP_SUBTRANSACTION(status = lua_pcall(L, 0, LUA_MULTRET, 1););
