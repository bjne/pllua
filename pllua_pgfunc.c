/*
    pgfunc works as a wrapper for postgres functions and as a loader
    as module for pllua (internal)->internal  functions.
...
static MemoryContext get_tmpcontext()
{
    ...
    mc = AllocSetContextCreate(
        TopMemoryContext, "pgfunc temporary context", ALLOCSET_DEFAULT_MINSIZE,
...
static int pg_callable_func(lua_State *L)
{
    ...
    for (i = 0; i < fi->numargs; ++i) {
        fcinfo.arg[i] =
            luaP_todatum(L, fi->argtypes[i], 0, &fcinfo.argnull[i], i + 1);
    }
...
static int pgfunc_rows(lua_State *L)
{
    Lua_pgfunc *fi;
    ReturnSetInfo *rsinfo;
    ExprContext *econtext;
    ...
    InitFunctionCallInfoData((*fcinfo), &srfi->fi, fi->numargs, InvalidOid,
                             NULL, (fmNodePtr)rsinfo);
...
int get_pgfunc(lua_State *L)
{
    ...
    if (lua_type(L, 1) == LUA_TSTRING) {
        ...
        PG_TRY();
        {
            funcid = DatumGetObjectId(
                DirectFunctionCall1(regprocedurein, CStringGetDatum(reg_name)));
        }
        PG_CATCH();
        {
        }
    ...
    if (opt.only_internal && (proc->prolang != INTERNALlanguageId) &&
        (!luasrc)) {
...

    {
        ...
        argc = get_func_arg_info(proctup, &argtypes, &argnames, &argmodes);
    ...
    if (luasrc) {
        ...
        if ((lf->numargs != 1) || (lf->argtypes[0] != INTERNALOID) ||
            (lf->prorettype != INTERNALOID)) {
            luaL_error(L, "pgfunc accepts only 'internal' pllua/u functions "
                          "with internal argument");
        }
        ...
        if (isnull) {
            elog(ERROR, "[pgfunc]: null lua prosrc");
        }
