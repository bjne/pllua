...
static void luaP_pushtuple_cmn(lua_State *L, HeapTuple tuple, int readonly,
                               RTupDesc *rtupdesc)
{
    luaP_Tuple *t;
    TupleDesc tupleDesc;
    ...
    t = lua_newuserdata(
        L, sizeof(luaP_Tuple) + n * (sizeof(Datum) + sizeof(bool)));
    ...
    for (i = 0; i < n; i++) {
        bool isnull;
        t->value[i] = heap_getattr(tuple, tupleDesc->attrs[i]->attnum,
                                   tupleDesc, &isnull);
        t->null[i] = isnull;
    }

    t->changed = readonly ? -1 : 0;
...
static luaP_Tuple *luaP_PTuple_rawctr(lua_State *L, HeapTuple tuple,
...
static int luaP_rowsaux(lua_State *L)
{
    ...
    if (c->tupleQueue == NULL) {
        ...
        PLLUA_PG_CATCH_RETHROW(SPI_cursor_fetch(c->cursor, 1, FETCH_CSR_Q););
...
luaP_Buffer *luaP_getbuffer(lua_State *L, int n)
{
    ...
    if (b == NULL || n > b->size) { /* resize? */
        ...
        b = (luaP_Buffer *)lua_newuserdata(
            L, sizeof(luaP_Buffer) + n * (sizeof(Datum) + sizeof(char)));
        b->size = n;
        b->value = (Datum *)(b + 1);
...
static void luaP_fillbuffer(lua_State *L, int pos, Oid *type, luaP_Buffer *b)
{
    lua_pushnil(L);
...
{
    luaP_Tuple **udata =
        (luaP_Tuple **)lua_newuserdata(L, sizeof(luaP_Tuple *));
...
static int luaP_p_tupleindex(lua_State *L)
{
    ...
    if (lua_type(L, 2) == LUA_TNUMBER) {
        ...
        if (t->rtupdesc) {
            ...
            if (tupleDesc == NULL) {
                ereport(
                    WARNING,
                    (errmsg("access to lost tuple desc at index %i", i + 1)));
                lua_pushnil(L);
            ...
            if ((i >= 0) && (i < tupleDesc->natts)) {
                if (!t->null[i]) {
                    luaP_pushdatum(L, t->value[i],
                                   tupleDesc->attrs[i]->atttypid);
                } else {
                    lua_pushnil(L);
                }
            } else {
                // ereport(WARNING, (errmsg("tuple has no field at index %i",
                // i+1)));
                // lua_pushnil(L);
    ...
    if (t->rtupdesc) {
        ...
        if (tupleDesc == NULL) {
            ereport(WARNING,
                    (errmsg("access to lost tuple desc at  '%s'", name)));
            lua_pushnil(L);
            return 1;
        }
                        ...
                        Oid relid, int readonly)
{
    ...
    for (i = 0; i < n; i++) {
        bool isnull;
        t->value[i] =
            heap_getattr(tuple, desc->attrs[i]->attnum, desc, &isnull);
        t->null[i] = isnull;
    }
...
HeapTuple luaP_casttuple(lua_State *L, TupleDesc tupdesc)
{
    ...
    for (i = 0; i < tupdesc->natts; i++) {
        ...
        if (j >= 0) {
            if (t->changed == -1) { /* read-only? */
                b->value[i] =
                    heap_getattr(t->tuple, t->tupdesc->attrs[j]->attnum,
                                 t->tupdesc, b->null + i);
            } else {
                b->value[i] = t->value[j];
                b->null[i] = t->null[j];
...
static int luaP_tuptableindex(lua_State *L)
{
    ...
    if (k > 0 && k <= t->size) {
        ...
        if (lua_isnil(L, -1)) { /* not interned? */
            ...
            luaP_pushtuple_cmn(L, t->tuptable->vals[k - 1], 1, t->rtupdesc);
...
static int luaP_executeplan(lua_State *L)
{
    ...
    if (((result == SPI_OK_SELECT) || (result == SPI_OK_UPDATE_RETURNING) ||
...
{
    luaP_Plan *p = (luaP_Plan *)luaP_checkudata(L, 1, PLLUA_PLANMT);
    PLLUA_PG_CATCH_RETHROW(p->plan = SPI_saveplan(p->plan););

    if (SPI_result == SPI_ERROR_ARGUMENT) {
        return luaL_error(L, "null plan to be saved");
    } else if (SPI_result == SPI_ERROR_UNCONNECTED) {
...
static int luaP_execute(lua_State *L)
{
    int result = -1;
    PLLUA_PG_CATCH_RETHROW(
        result = SPI_execute(luaL_checkstring(L, 1), (bool)lua_toboolean(L, 2),
#if LUA_VERSION_NUM >= 503
